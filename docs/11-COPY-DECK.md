...
`E35`. A cue is one short line, identical for every member, attached to some nights' rounds.
It changes what people drop, never how the game is scored, and it is neutral ink on every
surface (`docs/18-CUES.md` §2). The catalog below is the one source of truth: the seed
migration and `tests/db/cues.sql` both diff against it, so a cue must be edited here first.
...
| Key | String |
|---|---|
| `settings.cue.title` | Tonight's cue |
| `settings.cue.effective` | Starts %@. |
| `settings.cue.cadence.off` | Off |
...
| `group.member.report` | Report member |
| `group.member.report.title` | Report %@? |
| `group.member.report.body` | This goes to Blind Drop, not to the circle. |
| `group.member.report.reason.name` | Their display name |
| `group.member.report.reason.cue` | A cue they wrote |
| `group.member.report.reason.harassment` | Harassment |
| `group.member.report.reason.other` | Something else |
| `group.member.report.sent.title` | Report sent |
| `group.member.report.sent.body` | We'll look at it. Nothing changes in the circle for now. |
| `group.member.report.sent.action` | Done |
