...
| **Multiple groups per user** | Changes navigation, routing, and every API path (ADR-005) | No `group_id` parameter in any client-facing route. No group switcher. No array where a single group belongs. |
...
| **Web presence** | — | Nothing beyond the invite-link landing page (§3) |
...
- **Streaks, badges, XP, levels, cosmetics, avatars, profile customisation.** The two numbers
  are readability and ear. That is the entire progression system.
- **Notification settings.** Three pushes a day is already quiet
  (`05-JOBS-AND-NOTIFICATIONS.md` §4).
...
- **A local database.** The game is server-authoritative; a cache of game state is a
  correctness hazard.
- **A tab bar.** One primary action per screen.
