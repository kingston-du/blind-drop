...
/// `E24-01`. The circle's roster, ranked by all-time Ear — `GroupContent` is the value both the
...
enum GroupFixture {
    ...
    static func content(group: GroupDTO, standings: StandingsDTO) -> some View {
        ScrollView {
            GroupContent(
                ...
                isThinHistory: standings.roundsPlayed < GroupStore.thinHistoryThreshold,
                select: { _ in }
            )
    ...
    static func threeMembers(isAdmin: Bool) -> GroupDTO {
        ...
        {"id":"g_three","name":"Late Night Radio","timezone":"America/New_York",
         ...
         "members":[
           ...
           {"user_id":"u_ben","display_name":"Ben"},
           {"user_id":"u_cal","display_name":"Cal"}]}
        """)
    }
    ...
    static let twelveMembers: GroupDTO = {
        ...
        let members = names.enumerated().map { index, name in
            #"{"user_id":"u_\#(index)","display_name":"\#(name)"}"#
        }.joined(separator: ",")
