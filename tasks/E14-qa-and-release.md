...
**Status:** done · **Deps:** E04-04, E05-02 · **Reads:** `docs/14` §3, §10, `docs/15` AC-1
...
- [x] Every route in `functions/` has a golden file; a route without one fails the suite
- [x] Latency correlation with submitter count: |r| < 0.2 over 100 samples
- [x] Byte-length invariance across 0/1/5/11 other submitters
- [x] Accessibility labels on Submit and Sealed contain no digit but the countdown
- [x] No client model has a `submissionCount`, `hasSubmitted`, or `participants` field — grep,
      then fix
- [x] `GET /groups/current` still carries no `joined_at`
- [x] Rate limiting is per-user, never per-group (`docs/14` §3)
- [x] Authenticated and anonymous PostgREST calls to every table fail with `42501`
- [x] Write the pass/fail summary into `docs/` as a dated release note

---

### E14-02 — Accessibility pass

**Status:** done · **Deps:** E12-03, E13-01 · **Reads:** `docs/12` (all), `docs/15` §2
**Touches:** `BlindDropTests/Snapshot/*`, `BlindDropTests/Unit/A11y*`
**Verify:** the full snapshot matrix + `A11yReachabilityTests`

- [x] Five screens × `{large, a11y1, a11y5}` × `{SE, 15 Pro Max}` — no truncation, no overlap
- [x] Every element has a non-empty VoiceOver label (UI test walks each screen)
- [x] Every card announces number, title, artist, and current guess
- [x] Every hit region ≥ 44×44
- [x] SE × 12 members × `.accessibility5`: everything reachable
- [x] `PaletteContrastTests` green
- [x] Snapshot run with the OS in dark mode produces identical output to light
- [x] Reduced-motion final states pixel-identical to normal
- [x] Plurals via `.stringsdict` for every `%lld` count
- [x] Copy lint: no exclamation mark, no banned word

---

### E14-03 — Animation performance pass

**Status:** wip · **Deps:** E10-04, E11-04 · **Reads:** `docs/09` §6, `docs/15` AC-11
...
**Status:** done · **Deps:** E12-05 · **Reads:** `docs/15` AC-10, `docs/00` §7
...
- [x] Asserts total interaction time < 90s
- [x] Asserts tap count ≤ 18
- [x] Asserts first search results within 400ms of the second keystroke
- [x] Runs with a fake device clock set 5 years forward (AC-2) and with a foreign timezone
- [x] Runs the reduced-motion variant
- [x] Fails loudly with a per-step timing breakdown so a regression names its own cause

---

### E14-05 — Release checklist

**Status:** wip · **Deps:** E14-01, E14-02, E14-03, E14-04 · **Reads:** `docs/14` §10, `docs/15` §4
...
- [ ] Full iOS suite passes on both the minimum supported iOS 17.4 runtime and the current runtime
...
- [ ] `https://blinddrop.app/spotify-auth` registered in Spotify; the domain serves the signed
      app's AASA `webcredentials` entry; PKCE completes end to end on a real device
- [ ] Apple Music export creates an ordered playlist on a subscribed real device; denial and
      no-subscription states leave Spotify export available
