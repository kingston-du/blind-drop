...
**Verify:** `./ios/scripts/lint.sh`; unit + snapshot; `verify-fixture.sh` onboarding. Simulator:
create a circle and invite from it, on an SE.

**`+ Start a circle`** at the foot of the switcher. Creation asks for a name and the schedule,
and then gets out of the way — the second circle should cost a few taps, not a form. Timezone
defaults from the device and is fixed at creation, so it is stated rather than asked.

Then invitations, immediately, because a circle with one person in it is not a circle yet.
...
An existing user opening an invite link lands on a short Join screen — the circle's name, who
invited them, accept or not — never the sign-up flow they have already completed.

- [ ] `+ Start a circle` in the switcher; creation is name plus schedule, then invitations
- [ ] Timezone stated, not asked; reveal hour defaulted sensibly and adjustable
- [ ] People you've played with, derived from shared circles, no standalone list
